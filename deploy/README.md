...
- Azure SQL's serverless free tier **auto-pauses** after 60 minutes idle, and gets only **100,000 vCore-seconds (~55 hours) per month**. Do **not** add a keep-warm pinger: the app's `SeatLockCleanupService` queries the database every 2 minutes while the app is awake, so keeping the app warm keeps the database awake, and it burned the whole month's allowance in ~2.5 days (Azure then pauses the database until the 1st of the month — the site returns 503).
